...
export class InvoiceScheduler {
  ...
  async handleMonthlyInvoices() {
    ...
    const currentTime = jakartaTime.toLocaleTimeString('en-GB', {
      hour: '2-digit',
      minute: '2-digit',
    });
    ...
    this.logger.debug(
      `Schedule Config: Day ${schedule.day} @ ${schedule.time}`,
    );
    ...
    this.logger.log(
      `Schedule Match (Day ${schedule.day} @ ${schedule.time}). Generating monthly invoices...`,
    );
    const invoices = await this.paymentService.generateMonthlyInvoices();

    this.logger.log(
      `Generated ${invoices.length} invoices. Queuing notifications...`,
    );
  ...
  async handleInvoiceReminders() {
    ...
    this.logger.log(
      `Reminder Schedule Match (Day ${schedule.day} @ ${schedule.time}). Sending due date reminders...`,
    );
    ...
    if (overdueInvoices.length > 0) {
      this.logger.log(
        `Found ${overdueInvoices.length} unpaid invoices. Queuing reminders...`,
      );
