...
export function useBilling(
  ...
  const [manualLateTargetMonth, setManualLateTargetMonth] = useState<
    number | null
  >(null)
  const [manualLateTargetYear, setManualLateTargetYear] = useState<
    number | null
  >(null)
  const [manualLateExecutionDay, setManualLateExecutionDay] = useState<
    number | null
  >(null)
  ...
  const fetchManualLateSchedule = async () => {
    try {
      const res = await fetch(
        `${API_URL}/payment-module/manual-late-schedule`,
        {
          headers: getAuthHeaders(),
        },
      )
      ...
      const data = await res.json()
      setManualLateTargetMonth(data.targetMonth)
      setManualLateTargetYear(data.targetYear)
      setManualLateExecutionDay(data.executionDay)
      setManualLateExecutionTime(data.executionTime || "00:00")
      setManualLateIsActive(Boolean(data.isActive))
    } catch (error) {
      console.error("Failed to fetch manual late invoice schedule", error)
    }
  }

  const saveManualLateSchedule = async (config: {
    targetMonth: number
    targetYear: number
    executionDay: number
    ...
    try {
      ...
      const sendRes = await fetch(
        `${API_URL}/notifications/invoices/send-manual-late`,
        {
          method: "POST",
          headers: getAuthHeaders(),
          body: JSON.stringify({
            ...
            invoiceDay: config.executionDay,
            invoiceTime: config.executionTime,
          }),
        },
      )
      ...
      if (!sendRes.ok) {
        ...
        throw new Error(
          `Error ${sendRes.status}: ${sendRes.statusText} - ${text}`,
        )
      }

      const sendData = await sendRes.json()
      setManualLateIsActive(false)
      return {
        ...data,
        isActive: false,
        ...sendData,
      ...
      console.error("Failed to save manual late invoice schedule", error)
      throw error
    }
  }
  ...
  const deleteAllInvoices = async () => {
    try {
      const res = await fetch(
        `${API_URL}/payment-module/invoices/all?filter=${activeTab}`,
        {
          method: "DELETE",
          headers: getAuthHeaders(),
        },
