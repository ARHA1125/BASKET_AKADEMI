...
export default function StudentView() {
    ...
    const [applicationOrder, setApplicationOrder] = useState<'DESC' | 'ASC'>('DESC');
    ...
    const handleApprove = async (id: string) => {
        ...
        if (success) {
            ...
            fetchData(page, search, 10, { applicationOrder }); 
        } else {
    ...
    const handleBulkApprove = () => {
        toast("Approve pending students", {
            description: search
                ? `Approve all pending students matching "${search}"?`
                : 'Approve all pending students? ',
            action: {
                label: 'Approve All',
                onClick: async () => {
                    const result = await bulkApprovePending(search);
                    if (!result) {
                    ...
                    toast.success(`${result.updated} student${result.updated > 1 ? 's' : ''} approved and activated`);
                    fetchData(page, search, 10, { applicationOrder });
                }
            },
            cancel: { label: 'Cancel', onClick: () => {} }
    ...
    useEffect(() => {
        fetchData(page, search, 10, { applicationOrder });
    }, [page, search, applicationOrder, fetchData]);
    ...
    }, { scope: containerRef, dependencies: [filteredData, loading] });
    ...
    const pendingVisibleCount = filteredData.filter(student => student.user.status === 'Pending').length;
        ...
        <div className="space-y-8 font-sans">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-white p-1 dark:bg-slate-950 dark:border-slate-800">
                        <button
                            onClick={() => { setActiveTab('all'); setPage(1); }}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${activeTab === 'all' ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}
                        >
                            All
                        </button>
                        <button
                            onClick={() => { setActiveTab('pending'); setPage(1); }}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${activeTab === 'pending' ? 'bg-amber-500 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}
                        >
                            Pending
                        </button>
                        <button
                            onClick={() => { setActiveTab('active'); setPage(1); }}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${activeTab === 'active' ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}
                        >
                            Active
                        </button>
                    </div>
                    <select
                        ...
                        onChange={(e) => {
                            setApplicationOrder(e.target.value as 'DESC' | 'ASC');
                            setPage(1);
                        }}
                        className="px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:bg-slate-950 dark:border-slate-800 dark:text-white"
                    >
                        <option value="DESC">Recently Applying</option>
                        <option value="ASC">Earliest Applying</option>
                    </select>
                    <button
                        onClick={handleBulkApprove}
                        disabled={loading || bulkApproving || activeTab === 'active' || (search || activeTab === 'pending' ? pendingVisibleCount === 0 : stats.pending === 0)}
                        className="inline-flex items-center justify-center bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
            ...
            <Card className="overflow-hidden">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 border-b border-slate-200 dark:bg-slate-950 dark:border-slate-800">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold text-slate-900 dark:text-slate-200">Applied At</th>
                        ...
                        <tbody ref={containerRef} className="divide-y divide-slate-200 dark:divide-slate-800">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-8 text-center text-slate-500">No students found matching current filter.</td>
                                </tr>
                            ) : (
                                filteredData.map((item: Student) => (
                                    <tr key={item.id} className="gsap-table-row hover:bg-slate-50/50 transition-colors dark:hover:bg-slate-800/50 hover:-translate-y-0.5 hover:shadow-sm duration-200">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-slate-900 dark:text-slate-200">{formatAppliedAt(item.user.createdAt)}</div>
                                            <div className="text-slate-500 text-xs mt-0.5 dark:text-slate-500">
                                                {applicationOrder === 'DESC' ? 'Newest first' : 'Oldest first'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                ...
                <div className="border-t border-slate-200 px-6 py-4 bg-slate-50/50 flex justify-between items-center text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button 
                           disabled={allData.length < 10 && (page * 10 >= total)} 
