...
export default function ParentView() {
    const { data, loading, total, fetchData, createUnified, updateResource, deleteResource } = useParents();
    ...
    const handleDelete = async (id: string) => {
        toast("Delete Parent", {
            description: "Are you sure you want to delete this parent? This cannot be undone.",
            action: {
                label: "Delete",
                onClick: async () => {
                    const success = await deleteResource(id);
                    if (success) toast.success("Parent deleted");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (success) {
            ...
            resetForm();
            toast.success(`Parent ${editingId ? 'updated' : 'created'} successfully!`);
        } else {
            toast.error(`Failed to ${editingId ? 'update' : 'create'} parent.`);
        }
    };
    ...
    return (
        <div className="space-y-8 font-sans">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Parent Management</h1>
                    <p className="text-slate-500 mt-1 dark:text-slate-400">Manage parent accounts and their linked students.</p>
                </div>
                <button 
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 border-b border-slate-200 dark:bg-slate-950 dark:border-slate-800">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold text-slate-900 dark:text-slate-200">Students Linked</th>
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
                    <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col dark:bg-slate-900">
                        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900 dark:border-slate-800">
                            <div>
                                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{editingId ? 'Edit Parent' : 'Register New Parent'}</h2>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Creates a User account and Parent profile.</p>
                            </div>
                            <button 
                        ...
                        <div className="p-6 overflow-y-auto space-y-8">
                             <form id="create-parent-form" onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Full Name</label>
                                        <input type="text" name="fullName" required value={formData.fullName} onChange={handleInputChange} className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:text-white" placeholder="Jane Doe" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Email Address</label>
                                        <input type="email" name="email" required value={formData.email} onChange={handleInputChange} className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:text-white" placeholder="parent@example.com" />
                                    </div>
                                     <div className="space-y-1.5">
                        ...
                        <div className="p-6 border-t border-slate-200 bg-slate-50/50 flex justify-end gap-3 dark:bg-slate-900 dark:border-slate-800">
                             ...
                             <button type="submit" form="create-parent-form" className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors">
                                {editingId ? 'Update Profile' : 'Create Parent Profile'}
                             </button>
                        </div>
                    </div>
                </div>,
